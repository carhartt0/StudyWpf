...
namespace WpfNaverMovieFinder
{
    ...
    {
        bool IsFavorite = false;        //네이버 api로 검색한건지, 즐겨찾기 DB에서 온것인지 확인할 값
        // IsFavorite == true 면 DB에서 온 값, IsFavorite == false 면 네이버api
        ...
        {
            string clientID = "x0_cV5KvbQO0ca8uaEFY";
            string clientSecret = "yuI_Nkp28j";
        ...
        private void btnWatchTrailer_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            if (grdResult.SelectedItems.Count == 0)
            {
                Commons.ShowMessageAsync("유튜브영화영화", "영화를 선택하세요");
                return;
            }
            ...
            string movieName = string.Empty; //"" 이거랑 string.Empty 랑 똑같은 의미
        ...
        {
            if (grdResult.SelectedItem is MovieItem)        //네이버api에서 온 값이면
            {
            ...
            if(grdResult.SelectedItem is TblFavoriteMovies)     //즐겨찾기 DB값이면
            {
